...
public class Graphe {
	...
	public Graphe (String nomCarte, DataInputStream dis, Dessin dessin) {
		...
		try {
			...
			//____________________________********************
			som = new Sommet[nb_nodes] ;
			this.nb_noeuds = nb_nodes ;
			// for (int num_node = 0 ; num_node < nb_nodes ; num_node++) {
			// 	som[num_node] = new Sommet() ;
			// }
			//____________________________********************
			...
			for (int num_node = 0 ; num_node < nb_nodes ; num_node++) {
				...
				som[num_node] = new Sommet(longitudes[num_node], latitudes[num_node], num_node) ;
			}
			...
			for (int num_node = 0 ; num_node < nb_nodes ; num_node++) {
				...
				for (int num_succ = 0 ; num_succ < nsuccesseurs_a_lire[num_node] ; num_succ++) {
					...
					if(descripteurs[descr_num].isSensUnique()){
						...
						som[num_node].add(som[dest_node], longueur, descripteurs[descr_num]) ;

					}
					else {
						som[num_node].add(som[dest_node], longueur, descripteurs[descr_num]) ;
						som[dest_node].add(som[num_node], longueur, descripteurs[descr_num]) ;
					}
					//____________________________********************
	...
	public void verifierChemin(DataInputStream dis, String nom_chemin) {
		...
		try {
			...
			//____________________________********************
			chem = new Chemin(som[first_node], som[last_node], nb_noeuds) ;
			...
			System.out.println(chem.calcul());
